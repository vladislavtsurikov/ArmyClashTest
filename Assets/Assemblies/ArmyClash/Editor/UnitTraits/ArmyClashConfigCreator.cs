...
using System.IO;
using ArmyClash.UnitTraits;
using UnityEditor;
using UnityEngine;
using VladislavTsurikov.ActionFlow.Runtime.Stats;

namespace ArmyClash.Editor.UnitTraits
{
    public static class ArmyClashConfigCreator
    {
        ...
        private const string TraitsPath = RootPath + "/UnitTraits";
        private const string ShapesPath = TraitsPath + "/Shapes";
        private const string SizesPath = TraitsPath + "/Sizes";
        private const string ColorsPath = TraitsPath + "/Colors";

        [MenuItem("ArmyClash/Configs/Create Default Unit Trait Configs")]
        public static void CreateDefaults()
        {
            ...
            StatEffect cubeEffect = CreateEffect("Shape_Cube", new[]
            {
                (hp, 100f),
                (atk, 10f)
            });
            StatEffect sphereEffect = CreateEffect("Shape_Sphere", new[]
            {
                (hp, 50f),
                (atk, 20f)
            });

            StatEffect bigEffect = CreateEffect("Size_Big", new[]
            {
                (hp, 50f)
            });
            StatEffect smallEffect = CreateEffect("Size_Small", new[]
            {
                (hp, -50f)
            });

            StatEffect blueEffect = CreateEffect("Color_Blue", new[]
            {
                (atk, -15f),
                (atkSpd, 4f),
                (speed, 10f)
            });
            StatEffect greenEffect = CreateEffect("Color_Green", new[]
            {
                (hp, -50f),
                (atk, 20f),
                (speed, -5f)
            });
            StatEffect redEffect = CreateEffect("Color_Red", new[]
            {
                (hp, 200f),
                (atk, 40f),
                (speed, -9f)
        ...
        private static void EnsureFolders()
        {
            ...
            CreateFolderIfMissing(RootPath, "UnitTraits");
            CreateFolderIfMissing(TraitsPath, "Shapes");
            CreateFolderIfMissing(TraitsPath, "Sizes");
            CreateFolderIfMissing(TraitsPath, "Colors");
        }
        ...
        private static StatEffect CreateEffect(string name, (Stat stat, float delta)[] entries)
        {
            string assetPath = Path.Combine(EffectsPath, name + ".asset").Replace("\\", "/");
            StatEffect effect = AssetDatabase.LoadAssetAtPath<StatEffect>(assetPath);
            if (effect == null)
            {
                effect = ScriptableObject.CreateInstance<StatEffect>();
        ...
        private static void CreateShapeModifier(string id, string displayName, StatEffect effect, GameObject prefab)
        {
        ...
        private static void CreateSizeModifier(string id, string displayName, StatEffect effect, Vector3 scale)
        {
        ...
        private static void CreateColorModifier(string id, string displayName, StatEffect effect, Color color)
        {
